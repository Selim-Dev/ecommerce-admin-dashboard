...
const New = ({ title, input }) => {
  ...
  const token = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpZCI6IjYyNDI0ODdmNjMzNjJlMmRlOGZmNThjYyIsImlhdCI6MTY0ODUxMTI5NSwiZXhwIjoxNjU2Mjg3Mjk1fQ.xCiXj0S6RS3Kmbw40qds7rPTx8MqtEfJaONZZjbQ4l0"

  useEffect(() => {
    const getCategories = async () => {
      const categoreies = await axios.get('http://localhost:3000/api/v1/category');
  ...
  useEffect(() => {
    const getCategories = async () => {
      const Sellers = await axios.get('http://localhost:3000/api/v1/users?role=seller',
        {
          headers: {
  ...
  const onSubmit = async (data) => {
    ...
    console.log(data);
    const albumData = [];
    // data.album.album.map(item => albumData.push(item[0]))
  ...
  const handleCategoryChange = async (e) => {
    const subCategories = await axios.get(`http://localhost:3000/api/v1/category/${e.target.value}`);
              ...
              <Box sx={{ flexGrow: 3 }}>
                <Grid container spacing={2}>
                  ...
                  <Grid item xs={6}>
                    <label htmlFor="file">
                      Add Album:
                      <input  {...register('album', { required: true })} type="file" multiple />
                    </label>
                  ...
                  <Grid item xs={6}>
                    <FormControl fullWidth>
                      ...
                      >
                        ...
                        {Seller?.map(item => <MenuItem value={item._id}
                  ...
                  <Grid item xs={6}>
                    <FormControl fullWidth>
                      ...
                      <Select
                        ...
                        id="demo-simple-select-standard"
                        label="Category"
                        {...register('category', { required: true, onChange: (e) => handleCategoryChange(e) })}
                      >
                        <MenuItem value="">
                          <em>None</em>
                        </MenuItem>
                        {category?.map(item => <MenuItem value={item._id}
                  ...
                  <Grid item xs={6}>
                    <FormControl fullWidth>
                      ...
                      >
                        ...
                        {SubCategories?.map(item => <MenuItem value={item._id}
